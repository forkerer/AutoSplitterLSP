...
using Irony.Parsing;
using LiveSplit.LeanCore.Components;
using System.Collections.Generic;
using System.Linq;

namespace AutoSplitterLSP.LeanParserAndCompiler.ASL
{
    public class ASLParser
    {
        public static ASLScript Parse(string code)
        {
            ...
            if (tree.HasErrors())
            {
                var errors = parser.Context.CurrentParseTree.ParserMessages
                    .Select(x => new ErrorLocation
                    {
                        Line = x.Location.Line,
                        Column = x.Location.Column,
                        Position = x.Location.Position,
                        Message = x.Message,
                    });

                throw new ASLParseException(errors);
            }
